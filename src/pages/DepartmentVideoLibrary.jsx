...
function DepartmentVideoLibrary() {
  ...
  const getEditPath = (videoId) => {
    const role = getRole(currentUser);
    if (role === "superadmin") return `/super-admin/video-library/edit/${videoId}`;
    if (role === "admin") return `/admin/video-library/edit/${videoId}`;
    return `/department-admin/video-library/edit/${videoId}`;
  };
                ...
                {(canSeeAll || video.createdBy === currentUser?.id) && (
                  <div className="vl-card-actions">
                    <button
                      ...
                      onClick={(e) => { e.stopPropagation(); navigate(getEditPath(video.id)); }}
                      title="Edit"
                    >
